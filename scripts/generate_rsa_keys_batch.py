...
def generate_rsa_key_pair(username: str, output_dir: Path, key_size: int = 2048) -> tuple[Path, Path]:
    """
    ...
    try:
        ...
        genrsa_result = subprocess.run(genrsa_cmd, capture_output=True, text=True, check=True)
...
def main():
    ...
    parser.add_argument(
        '--users',
        nargs='+',
        help='List of usernames to generate keys for'
    )
    parser.add_argument(
        '--all-non-compliant',
        action='store_true',
    ...
    print("  1. Store private keys in secure locations (1Password, AWS Secrets Manager)")
